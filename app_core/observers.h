...
public:
    ...
    void run() override {
        ...
        while (!mFinish) {
            ...
            emit handleResults(mResult[mIdx].mErrorCode, mResult[mIdx].mOperation, mResult[mIdx].mExactSqrt, res, mIdx);
            mFree->release();
        }
    }
...
signals:
    void handleResults(int, int, bool, QVector<dec_n::Decimal>, int);
